#define UART_THR_ADDR   0x10000000; // UART Transmitter Holding Register, when DLAB disabled
#define UART_IER_ADDR   0x10000001; // UART Interrupt Enable Register, when DLAB disabled
...
#define UART_DL_LS_ADDR 0x10000000; // UART Divisor Latch, LSB, when DLAB enabled
#define UART_DL_MS_ADDR 0x10000001; // UART Divisor Latch, MSB, when DLAB enabled
...
void setup_uart() {
    ...
    *UART_DL_MS = 0x00;
    ...
    // Setup LCR
    *UART_LCR = 0x03; // 8-bit characters, one stop bit, no parity check; disable DLAB
